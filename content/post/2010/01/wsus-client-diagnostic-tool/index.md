---
title: "WSUS Client Diagnostic Tool"
layout: "post"
date: 2010-01-19T20:18:05Z
slug: "wsus-client-diagnostic-tool"
aliases:
  - "/2010/01/wsus-client-diagnostic-tool/"
description: "Having trouble with a client not getting updates from your Windows Update Services Server ? Then have a look at the WSUS Client Diagnostics Tool. The ..."
author: "Alex Verboon"
image: "img/post-heroes/wsus-client-diagnostic-tool.png"
tags:
  - bits
  - diagnostic
  - patch-management-software
  - troubleshooting
  - wsus
  - Windows
categories:
  - bits
  - patch-manegement-software
  - security
  - tip
  - tools
  - windows-update-services
  - wsus
---
Having trouble with a client not getting updates from your Windows Update Services Server ? Then have a look at the WSUS Client Diagnostics Tool.  The tool performs various system checks and tests the communication between your client and the WSUS server. 

  [
![wsuscltool](images/wsuscltool_thumb.png)
](https://www.verboon.info/wp-content/uploads/2010/01/wsuscltool.png) 

  The Tool can be downloaded from the [Windows Server Update Services Tools and Utilities](http://technet.microsoft.com/en-us/wsus/bb466192.aspx) site at Microsoft TechNet.

