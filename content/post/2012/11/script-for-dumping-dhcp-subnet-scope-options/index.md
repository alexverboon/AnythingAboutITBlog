...
Copy the code below and save it as `dhcpsubnetscopeoptions.cmd`.
...
for /f "skip=4 tokens=1 delims= " %%a in ('netsh.exe dhcp server %srv% show scope') DO (
  IF "%%a" == "Total" (echo.) ELSE IF "%%a" == "Command" (echo.) ELSE (
    netsh dhcp server %srv% scope %%a show optionvalue >>%~dp0\dhcpsubnetscopeoptions.txt
  )
)
```
