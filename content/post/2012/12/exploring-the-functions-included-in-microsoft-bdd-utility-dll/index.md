...
```
  ...
  | HiddenPartitionsToDrives | Creates an array of all local partitions, including hidden ones such as the Recovery partition |
  | Is64Bit | If the processor is running a x86 OS, then there is *still* the possibility that it can support a x64 OS. We need to run a quick processor check to see if it supports x64. |
  | IsAdmin | Validate we are running as an administrator |
  | IsHypervisorRunning | Get virtualization details |
  | IsKnownToDNS | *(no references found in scripts and couldn't figure how to call this function)* |
  | IsUEFI | Detect whether system uses UEFI or BIOS |
  | KeepAlive | Used in MDT scripts to prevent the system from going into sleep while the task sequence is running. |
  | Supports64Bit | Checks whether system supports 64 Bit |
  | SupportsNX | Checks whether system supports NX |
  | SupportsVT | Checks whether the system supports Virtualization Technology |
